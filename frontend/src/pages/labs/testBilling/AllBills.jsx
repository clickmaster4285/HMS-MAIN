...
const AllBills = () => {
  ...
  const filteredBills = allBills.filter(bill => {
    ...
    // Patient name filter
    if (filters.patientName && filters.patientName.trim()) {
      if (!bill.patientName.toLowerCase().includes(filters.patientName.toLowerCase())) {
        return false;
      }
    }
    ...
    // Patient MRNo filter
    if (filters.patientMRNo && filters.patientMRNo.trim()) {
      if (!bill.patientMRNo.toLowerCase().includes(filters.patientMRNo.toLowerCase())) {
        return false;
      }
    }
    ...
    // Status filter
    if (filters.status && filters.status.trim()) {
      const hasStatus = bill.tests.some(test => test.status === filters.status);
    ...
    if (filters.startDate) {
      ...
      if (filters.endDate) {
        if (billDate < filters.startDate || billDate > filters.endDate) return false;
      } else {
        const sameDay = 
  ...
  const calculateSummary = () => {
    ...
    const totalPaid = filteredBills.reduce((sum, bill) => sum + bill.totalPaid, 0);
    const totalRemaining = filteredBills.reduce((sum, bill) => sum + bill.remainingAmount, 0);
    const totalDiscount = filteredBills.reduce((sum, bill) => sum + bill.discountAmount, 0);
    const totalAdvance = filteredBills.reduce((sum, bill) => sum + bill.advanceAmount, 0);
  ...
  useEffect(() => {
    if (labStatus === 'failed' && labError) {
      toast.error(`Failed to load lab bills: ${labError.message || 'Unknown error'}`);
    }
    
    if (radiologyStatus === 'failed' && radiologyError) {
      toast.error(`Failed to load radiology bills: ${radiologyError.message || 'Unknown error'}`);
    }
    ...
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      {(labStatus === 'failed' || radiologyStatus === 'failed') && (
        <div className="p-4 bg-red-100 border-l-4 border-red-500 text-red-700 rounded-md mb-6">
          ...
          <p className="mt-2">
            {labError?.message || radiologyError?.message || 'An unknown error occurred while fetching bills.'}
          </p>
          <button
            onClick={() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-primary-50 p-4 rounded-lg border border-primary-100">
          ...
          <p className="text-2xl font-bold text-primary-800">{summary.totalBills}</p>
          <p className="text-xs text-primary-500 mt-1">{summary.totalTests} tests</p>
        </div>
            ...
            {currentBills.length > 0 ? (
              currentBills.map((bill) => (
                <tr key={bill.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${paymentStatusColors[bill.paymentStatus]}`}>
