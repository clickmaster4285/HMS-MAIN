   ...
   try {
      ...
      const [doctors, opdPatients, ipdRecords, refunds] = await Promise.all([
         ...
         hospitalModel.AdmittedPatient.find({
            deleted: false,
            'admission_Details.admission_Date': { $gte: start, $lt: end }
      ...
      ['xray', 'ecg', 'bsr', 'labtest', 'consultation', 'other'].forEach(cat => {
         purposeStats[cat] = { count: 0, revenue: 0 };
      });
      ...
      ipdRecords.forEach(record => {
         ...
         const status = financials.status || 'Unknown';
      ...
      const response = {
         dateRange: {
            ...
            end: endDate || start.toISOString().split('T')[0], // Use provided endDate or start date
            isToday: !startDate && !endDate // True only when no dates provided
         },
         overview: {
         ...
         opd: {
            statistics: {
               ...
               todayPatients: opdStats.today, // Actually means "patients in date range"
               withDoctor: opdStats.withDoctor,
               withoutDoctor: opdStats.withoutDoctor,
               totalRevenue: opdStats.revenue, // Actually means "revenue in date range"
               todayRevenue: opdStats.todayRevenue, // Same as totalRevenue
